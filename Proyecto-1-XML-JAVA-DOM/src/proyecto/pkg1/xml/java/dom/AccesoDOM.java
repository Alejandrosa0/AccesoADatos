...
public class AccesoDOM {
    ...
    public int insertarLibroEnDOM() { //Función para insertar un nuevo libro en DOM
        String id, author, title, genre, price, publish_date, description; //Variables
        try {
            Scanner scanner = new Scanner(System.in);
            
            System.out.print("Introduce el ID del nuevo libro: "); //Pedimos que introduzca el ID por teclado
            id = scanner.nextLine();           
            
            System.out.print("Introduce el Autor del nuevo libro: "); //Pedimos que introduzca el Autor por teclado y valida que solo sea texto sino lo vuelve a pedir
            ...
            System.out.print("Introduce el Titulo del nuevo libro: "); //Pedimos que introduzca el Título por teclado y valida que solo sea texto sino lo vuelve a pedir
            ...
            System.out.print("Introduce el Gérero del nuevo libro: ");  //Pedimos que introduzca el Género por teclado y valida que solo sea texto sino lo vuelve
            ...
            System.out.print("Introduce el Precio del nuevo libro: "); //Pedimos que introduzca el Precio por teclado
            price = scanner.nextLine();
            
            System.out.print("Introduce la Fecha De Publicación del nuevo libro: "); //Pedimos que introduzca la Fecha De Publicación por teclado
            publish_date = scanner.nextLine();
            
            System.out.print("Introduce la Descripción del nuevo libro: "); //Pedimos que introduzca la descripción por teclado
        ...
        try { //Crea los nodos para el DOM y inserta el libro en el DOM en orden
            ...
            nbook.appendChild(ntitulo); //Lista de nodos hijos
            ...
            //APPEND LIBRO A LA RAÍZ EN ORDEN
            Node raiz = doc.getFirstChild();
            NodeList nodelist = raiz.getChildNodes();
            boolean inserted = false;
            for (int i = 0; i < nodelist.getLength(); i++) { //Recorre la lista de nodos hijos
                Node nodo = nodelist.item(i);
                if (nodo.getNodeType() == Node.ELEMENT_NODE) { //Si es nodo de tipo ELEMENT_NODE
                    String nodoId = nodo.getChildNodes().item(0).getTextContent(); //Obtiene el Id del nodo
                    if (nodoId.compareTo(id) > 0) { //Se comparan los ids del nodo y del nuevo libro para ver si se inserta antes el nuevo libro o despues del nodo que está actualmente
            ...
            if (!inserted) { //Si no se ha insertado el libro se añadirá al final de la raíz
                raiz.appendChild(nbook);
            }
            System.out.println("Libro insertado en DOM.");
            return 0;
        } catch (Exception e) { //Excepción Exception e y su mensaje de error
            System.out.println(e);
            return -1;
        }
    }
